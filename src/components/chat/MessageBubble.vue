...
const role = computed(() => props.message.role ?? props.message.sender ?? 'assistant')
const isUser = computed(() => String(role.value).toLowerCase() === 'user')

const content = computed(() => props.message.content ?? props.message.text ?? '')

const rawTs = computed(() =>
  props.message.timestamp ??
  props.message.createdAt ??
  props.message.created_at ??
  props.message.regstamp ??
  null
)

const formattedTime = computed(() => {
  if (!rawTs.value) return ''
  const date = new Date(rawTs.value)
...
const bubbleClass = computed(() => {
  const base = 'px-5 py-3.5 rounded-2xl shadow-lg max-w-[75%] backdrop-blur-sm transition-all duration-300 hover:scale-[1.02]'
  return isUser.value
    ? `${base} bg-gradient-to-br from-meteora-cyan/90 to-blue-500/90 text-white border border-meteora-cyan/20`
    : `${base} bg-neutral-800/80 text-gray-100 border border-neutral-700/50`
})
</script>
